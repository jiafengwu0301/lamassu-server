...
const car = require('../coinatmradar/coinatmradar')
...
function status () {
  ...
  return Promise.all([checkWasConfigured(), db.oneOrNone(sql, ['ping']), machinesLastPing()])
    .then(([wasConfigured, statusRow, machineStatus]) => {
      ...
      const status = settingsLoader.loadLatest()
        .catch(() => null)
        .then(settings => {
          const config = settings.config
          return car.mapRecord(config)
            .then(machineRealStatus => {
              return getRates(settings)
                .then(rates => ({wasConfigured, up, lastPing, rates, machineStatus,machineRealStatus}))
            })
